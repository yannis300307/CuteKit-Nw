...
use crate::renderer::{
    ...
    mesh::{FlatMeshTriangle, FlatTriangle2D},
};

pub fn flat_triangle_clip_against_plane(
    plane_p: &Vector3<f32>,
    plane_n: &Vector3<f32>,
    in_tri: &FlatMeshTriangle,
    verticies: &mut Vec<Vector3<f32>>,
) -> (
    (Option<FlatMeshTriangle>, Option<FlatMeshTriangle>),
    ...
    if n_inside_point_count == 1 && n_outside_point_count == 2 {
        ...
        let (extra_v2, _t) = vector_intersect_plane(
            plane_p,
            &plane_n,
        ...
        let (extra_v3, _t) = vector_intersect_plane(
            plane_p,
            &plane_n,
            &verticies[inside_points[0] as usize],
    ...
    if n_inside_point_count == 2 && n_outside_point_count == 1 {
        let (extra_v3_1, _t) = vector_intersect_plane(
            plane_p,
            &plane_n,
            &verticies[inside_points[0] as usize],
            &verticies[outside_points[0] as usize],
        ...
        let (extra_v3_2, _t) = vector_intersect_plane(
            plane_p,
            &plane_n,
...
pub fn flat_triangle_clip_against_line(
    line_p: &Vector2<f32>,
    line_n: &Vector2<f32>,
    in_tri: &FlatTriangle2D,
) -> (Option<FlatTriangle2D>, Option<FlatTriangle2D>) {
    ...
    let default = Default::default();
    let mut inside_tex: [&f32; 3] = [&default; 3];
    let mut n_inside_tex_count = 0;
    let mut outside_tex: [&f32; 3] = [&default; 3];
    ...
    if d0 >= 0.0 {
        ...
        inside_tex[n_inside_tex_count] = &in_tri.depth.0;
        n_inside_tex_count += 1;
        n_inside_point_count += 1;
    } else {
        outside_points[n_outside_point_count] = &p1;
        outside_tex[n_outside_tex_count] = &in_tri.depth.0;
        n_outside_tex_count += 1;
        n_outside_point_count += 1;
    }
    if d1 >= 0.0 {
        inside_points[n_inside_point_count] = &p2;
        inside_tex[n_inside_tex_count] = &in_tri.depth.1;
        n_inside_tex_count += 1;
        n_inside_point_count += 1;
    } else {
        outside_points[n_outside_point_count] = &p2;
        outside_tex[n_outside_tex_count] = &in_tri.depth.1;
        n_outside_tex_count += 1;
        n_outside_point_count += 1;
    }
    if d2 >= 0.0 {
        inside_points[n_inside_point_count] = &p3;
        inside_tex[n_inside_tex_count] = &in_tri.depth.2;
        n_inside_tex_count += 1;
        n_inside_point_count += 1;
    } else {
        outside_points[n_outside_point_count] = &p3;
        outside_tex[n_outside_tex_count] = &in_tri.depth.2;
        n_outside_tex_count += 1;
        n_outside_point_count += 1;
    }
    ...
    if n_inside_point_count == 1 && n_outside_point_count == 2 {
        ...
        let out_tri = FlatTriangle2D {
            p1: p1.map(|x| x as i16),
            p2,
            p3,
            depth: (*inside_tex[0], t2, t3),
            color: in_tri.color,
        };
    ...
    if n_inside_point_count == 2 && n_outside_point_count == 1 {
        ...
        let out_tri1 = FlatTriangle2D {
            p1: p1.map(|x| x as i16),
            p2: p2.map(|x| x as i16),
            p3,
            depth: (*inside_tex[0], *inside_tex[1], t3),
            color: in_tri.color,
        };
        ...
        let out_tri2 = FlatTriangle2D {
            p1: inside_points[1].map(|x: f32| x as i16),
            p2: out_tri1.p3,
            p3,
            depth: (*inside_tex[1], out_tri1.depth.2, t3),
            color: in_tri.color,
        };
