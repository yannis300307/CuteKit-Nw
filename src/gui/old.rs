...
enum ChildrenType<'a, 'b> {
    ContainerNode(&'b [&'a dyn ContainerNode<'a>]),
    Nodes(&'b [NodeType<'a>]),
        ...
        if let Some(size) = self.size {
            ...
            for child in self.children {
                let size = match child {
                    NodeType::Node(node) => node.get_size(),
...
impl<'a> Menu<'a> {
    fn add_child_to_queue<'b, const SIZE: usize>(
        ...
        match parent.get_layout_ovewrite() {
        ...
        match children {
            ...
            ChildrenType::Nodes(nodes) => {
                ...
                for child in nodes.iter() {
                    match child {
                        NodeType::Node(node) => {
                            self.render_object(draw_queue, *node, &mut sub_start_offset)?
                        }
